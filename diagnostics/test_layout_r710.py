...
with sync_playwright() as p:
 ...
 for w,h in [(320,640),(390,844),(412,915),(640,360),(1280,900)]:
  ...
  check(f'{w}x{h} source and release identity',lambda: {'build':geometry(page)['build'],'footer':geometry(page)['footer']} if geometry(page)['build']=='r710' and 'r710' in geometry(page)['footer'] and not errors else (_ for _ in ()).throw(AssertionError(errors or 'release mismatch')))
  ...
  if w==390:
   ...
   page.evaluate('window.scrollTo(0,0)');page.screenshot(path=str(P/'diagnostics/r710_full_html_390x844.png'))
  if errors:results.append({'name':f'{w}x{h} uncaught browser errors','status':'FAIL','error':errors})
  c.close()
 browser.close()
out={'kind':'r710-full-HTML-controlled-Chromium','environment':'Actual application HTML via set_content; artwork supplied as local data URI; network blocked; visual playback fixture; no physical Android or audible audio','total':len(results),'passed':sum(x['status']=='PASS' for x in results),'failed':sum(x['status']=='FAIL' for x in results),'results':results}
(P/'diagnostics/r710_layout_results.json').write_text(json.dumps(out,ensure_ascii=False,indent=2)+'\n')
