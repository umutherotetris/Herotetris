...
(()=>{
'use strict';
if(window.SukunAudioTruth?.version==='r637')return;
const VERSION='r637';
...
function derive(reason='snapshot'){
  ...
  const phy=physical();
  const sessions=safe(()=>R?.list?.().map(id=>R?.sessionSnapshot?.(id)).filter(Boolean),[])||[];
  ...
  const tangible=nonMixPlaying||phy.journey28||phy.journey99||phy.terkip||phy.lock||phy.speech||phy.autoVoice||phy.background||phy.media>0||phy.tts>0;
  ...
  else if(nonMixPaused||norm(ctl?.state)==='paused')canonical='paused';
