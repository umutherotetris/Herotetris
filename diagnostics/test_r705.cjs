...
(async()=>{const result={kind:'r705-isolated-source-regression',date:new Date().toISOString(),target:await suite(target)};if(baseline)result.baseline=await suite(baseline);console.log(JSON.stringify(result,null,2));process.exitCode=result.target.failed?1:0})().catch(e=>{console.error(e);process.exitCode=1});
