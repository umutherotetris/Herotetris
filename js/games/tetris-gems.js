...
export const GEMS = [
  { id:'bomb',    c:'#FF5252', ic:'💣',  nm:'BOMBA',     desc:'5x5 patlama',
    run:(api)=>{ const cx=Math.floor(api.COLS/2); let n=0; for(let r=api.ROWS-7;r<api.ROWS;r++) for(let c=cx-3;c<=cx+3;c++){ if(r>=0&&c>=0&&c<api.COLS&&api.board[r][c]){api.board[r][c]=0;n++;} } api.addScore(n*20+350); return 'BOMBA! '+n; } },
  ...
  { id:'double',  c:'#FFD740', ic:'✨',  nm:'x2 PUAN',   desc:'Anında bonus puan',
    run:(api)=>{ api.addScore(750); return 'x2 PUAN! +750'; } },
  { id:'web',     c:'#F44336', ic:'🕸️', nm:'ÖR. AĞI',   desc:'8 sn yavaş düşüş',
    run:(api)=>{ api.slowTime(8000); api.addScore(80); return 'ÖRÜMCEK AĞI!'; } },
  ...
  { id:'tilt',    c:'#FF9800', ic:'↩️', nm:'TİLT',      desc:'Bloklar sola kayar',
    run:(api)=>{ for(let r=0;r<api.ROWS;r++){ const row=api.board[r].filter(c=>c); while(row.length<api.COLS) row.push(0); api.board[r]=row; } api.addScore(150); return 'TİLT!'; } },
  { id:'thunder', c:'#2196F3', ic:'⚡',  nm:'GÖK GÜRÜLTÜSÜ', desc:'Alttaki 5 sırayı yok et',
    run:(api)=>{ let n=0; for(let i=0;i<5;i++){ if(api.board[api.ROWS-1].some(c=>c)){ api.board.splice(api.ROWS-1,1); api.board.unshift(api.emptyRow()); n++; } } api.addScore(n*100+200); return 'GÖK GÜRÜLTÜSÜ!'; } },
  ...
  { id:'stardust',c:'#E040FB', ic:'✦',   nm:'YILDIZ TOZU', desc:'Büyük puan patlaması',
    run:(api)=>{ api.addScore(900); return 'YILDIZ TOZU! +900'; } },
  ...
  { id:'smash',   c:'#4CAF50', ic:'🪨',  nm:'DEV YUMRUK', desc:'Alttaki 4 sırayı ez',
    run:(api)=>{ let n=0; for(let i=0;i<4;i++){ if(api.board[api.ROWS-1].some(c=>c)){ api.board.splice(api.ROWS-1,1); api.board.unshift(api.emptyRow()); n++; } } api.addScore(n*120+250); return 'DEV YUMRUK!'; } },
...
export const HERO_GEM_BIAS = {
  hiz:   { chance:0.3,  ids:['phase'] },
  golem: { chance:0.3,  ids:['smash','gravity'] },
  pal:   { chance:0.25, ids:['chaos'] },
  neks:  { chance:0.2,  ids:['phase','magnet','gravity'] },
  vor:   { chance:0.2,  ids:['thunder'] },
  ksen:  { chance:0.2,  ids:['stardust'] },
  gol:   { chance:0.2,  ids:['bomb'] },
  ELARA: { chance:0.2,  ids:['double'] },
  arak:  { chance:0.2,  ids:['web'] }
};
