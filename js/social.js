...
/* ─── MESAJ DÜZENLEME / SİLME / QUICK BAR ─────────────────── */
...
function _editLabel(m){
  ...
  if(m.editHistory&&m.editHistory.length){
    const items=m.editHistory.map((h,i)=>'<div style="font-size:10px;color:#b0bec5;padding:2px 0">'+'<span style="color:#FFD740;font-weight:800;margin-right:4px">'+(i+1)+'.</span>'+esc(h.text||'')+' <span style="color:#546e7a;font-size:9px">'+(h.at?tAgo(h.at):'')+' </span></div>').join('');
...
function _actBtns(key,m,myUid){
  ...
  if(!e&&!d) return '';
  const eB=e?'<button data-ekey="'+key+'" title="D\u00fczle (5 dk)" style="background:none;border:none;cursor:pointer;font-size:13px;padding:2px 5px;border-radius:5px;color:#4FC3F7;opacity:.75;line-height:1">\u270f\ufe0f</button>':'';
...
export function initSocial(){
    ...
    <div class="ghp-body">
      <div class="ghp-pane active" id="ghpPane-chat">
        ...
        <div class="ghp-quick-bar" id="ghpChatQuick">
          <button class="ghp-qb" data-q="😂">😂</button><button class="ghp-qb" data-q="❤️">❤️</button><button class="ghp-qb" data-q="👍">👍</button><button class="ghp-qb" data-q="🔥">🔥</button><button class="ghp-qb" data-q="😮">😮</button><button class="ghp-qb" data-q="😢">😢</button><button class="ghp-qb" data-q="🎉">🎉</button><button class="ghp-qb" data-q="💪">💪</button><span class="ghp-qb-sep"></span><button class="ghp-qb ghp-qb-text" data-q="GG">GG</button><button class="ghp-qb ghp-qb-text" data-q="Merhaba! 👋">Mrb</button><button class="ghp-qb ghp-qb-text" data-q="Tebrikler! 🎉">Tebrik</button><button class="ghp-qb ghp-qb-text" data-q="İyi oyunlar! ⚔️">İyi oyun</button>
        </div>
      ...
      <div class="ghp-pane" id="ghpPane-ozel">
        ...
        <div id="ghpDMThread" style="display:none;flex-direction:column;flex:1;overflow:hidden">
          ...
          <div class="ghp-quick-bar" id="ghpDMQuick">
            <button class="ghp-qb" data-dq="😂">😂</button><button class="ghp-qb" data-dq="❤️">❤️</button><button class="ghp-qb" data-dq="👍">👍</button><button class="ghp-qb" data-dq="🔥">🔥</button><button class="ghp-qb" data-dq="😮">😮</button><button class="ghp-qb" data-dq="😢">😢</button><button class="ghp-qb" data-dq="🎉">🎉</button><button class="ghp-qb" data-dq="💪">💪</button><span class="ghp-qb-sep"></span><button class="ghp-qb ghp-qb-text" data-dq="GG">GG</button><button class="ghp-qb ghp-qb-text" data-dq="Merhaba! 👋">Mrb</button><button class="ghp-qb ghp-qb-text" data-dq="Tebrikler! 🎉">Tebrik</button><button class="ghp-qb ghp-qb-poke" id="ghpDMPoke">👋 Dürt</button>
          </div>
  ...
  // ── CSS: quick bar + hover aksiyon butonları
  if(!document.getElementById('_ghp_ex_css')){
    const _xs=document.createElement('style'); _xs.id='_ghp_ex_css';
    _xs.textContent=`
      ...
      .ghp-chat-row:hover ._macts,.ghp-chat-row:focus-within ._macts{opacity:1!important;}
    `;
  ...
  // ── Quick emoji/kalıp — Chat
  panel.querySelectorAll('#ghpChatQuick [data-q]').forEach(btn=>{
    btn.addEventListener('click',()=>{
      const inp=byId('ghpChatInput'); if(!inp) return;
      inp.value=(inp.value?inp.value+' ':'')+btn.dataset.q;
      inp.focus(); sendChat();
    });
  });
  // ── Quick emoji/kalıp — DM
  panel.querySelectorAll('#ghpDMQuick [data-dq]').forEach(btn=>{
    btn.addEventListener('click',()=>{
  ...
  if(_pokeBtn) _pokeBtn.addEventListener('click',async()=>{
    ...
    try{
      await fdb.push(fdb.ref(db,'userNotifs/'+H.dmThread.uid),{
        type:'poke',icon:'👋',
        text:(me2.displayName||me2.profile?.nick||'Biri')+' seni dürtüyor! 👋',
        ts:Date.now(), fromUid:me2.uid
      });
    }catch(e){}
...
function dmOpenThread(uid, nick){
  ...
  function renderDMRows(rows){
    ...
    box.innerHTML=rows.map(m=>{
        ...
        return `${dateSep}<div class="ghp-chat-row${isMine?' mine':''}" style="align-items:flex-end;gap:6px">
          ...
          <div style="flex:1;min-width:0">
            <div style="background:rgba(30,30,50,.4);border:1px solid rgba(80,80,100,.3);border-radius:12px;padding:6px 10px;max-width:82%;${isMine?'margin-left:auto':''}">
              <span style="color:#546e7a;font-style:italic;font-size:12px">Bu mesaj silindi.</span>
            </div>
      ...
      const txt=m.deleted?'<span style="color:#546e7a;font-style:italic">🗑 Silindi (admin görüntülüyor)</span>':esc(m.text);
