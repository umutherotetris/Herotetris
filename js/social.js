...
export function initSocial(){
  ...
  panel.innerHTML = `
    ...
    <div class="ghp-body">
      ...
      <div class="ghp-pane" id="ghpPane-lider">
        <div class="ghp-input-row" style="border-top:none;border-bottom:1px solid rgba(255,215,64,.12);justify-content:center;gap:6px">
  ...
  // 🏆 Liderlik alt sekmeleri (XP / Seviye)
  panel.querySelectorAll('.ghp-lb-tab').forEach(b =>
...
async function renderLeaderboard(mode){
  ...
  try{
    ...
    if(snap.exists()){
      snap.forEach(ch => {
        ...
        users.push({
          uid: ch.key,
          nick: v.nick || v.name || 'Oyuncu',
          level: Number(v.level || 1),
          xp: Number(xp) || 0,
          avatar: avatarOf(v, ch.key),
          isAdmin: v.isAdmin === true || (H.admins && H.admins[ch.key])
        });
      });
    }
  }catch(e){ console.warn('lb err', e); }
  if(!users.length){
    list.innerHTML = '<div class="ghp-empty"><div class="ghp-empty-icon">🏆</div><div class="ghp-empty-text">VERİ YOK</div></div>';
    return;
  }
  // Sırala
  ...
  list.innerHTML = top.map((u, i) => {
    ...
    const admBadge = u.isAdmin ? '<span style="font-size:9px;background:rgba(255,215,64,.2);border:1px solid rgba(255,215,64,.4);border-radius:4px;padding:0 3px;margin-left:3px;color:#FFD740">👑</span>' : '';
    return `<div class="ghp-lb-row" data-pclb="${esc(u.uid)}" style="display:flex;align-items:center;gap:9px;padding:8px 11px;border-bottom:1px solid rgba(255,255,255,.04);cursor:pointer;${isMe?'background:rgba(0,229,255,.06)':''}">
      <div style="width:26px;text-align:center;flex-shrink:0">${medal(i)}</div>
      <div style="width:30px;height:30px;border-radius:50%;background:rgba(30,30,60,.7);display:flex;align-items:center;justify-content:center;font-size:17px;flex-shrink:0">${esc(u.avatar)}</div>
      <div style="flex:1;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size:13px;font-weight:700;color:${nameCol}">${esc(u.nick)}${admBadge}${isMe?' <span style="font-size:9px;color:#00E5FF">(sen)</span>':''}</div>
      <div style="font-size:12px;font-weight:800;color:#FFD740;flex-shrink:0">${val}</div>
    </div>`;
  }).join('');
  list.querySelectorAll('[data-pclb]').forEach(el =>
    el.addEventListener('click', () => openPlayerCard(el.dataset.pclb)));
}
