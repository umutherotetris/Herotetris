...
function buildGameDOM(){
  // Portal HUD enjekte et (kl-top hemen altına)
  const _kltop = G.root.querySelector('.kl-top');
  if(_kltop && !G.root.querySelector('#heroGameHud')){
    try{
      const _as = window.Hero&&window.Hero.Auth&&window.Hero.Auth.getState&&window.Hero.Auth.getState();
      const _ps = window.Hero&&window.Hero.Store&&window.Hero.Store&&window.Hero.Store.getState&&window.Hero.Store.getState();
      ...
      import('../hud.js').then(H=>{
        H.createVsHUD({
          root: G.root, game:'kelime', myAccent:'#69F0AE', oppAccent:'#c084fc',
