      ...
      <div class="clan-card clan-banner">
        <div style="display:flex;align-items:center;gap:12px;min-width:0">
          ...
          <div style="flex:1;min-width:0;overflow:hidden">
            <div style="font-size:16px;font-weight:900;color:#ffd86b;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${esc(_clanData.name||'Klan')}</div>
            <div style="font-size:11px;color:#aab6da;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">👥 ${members.length} üye · 💰 ${fmt(_clanData.kaju)} · 🏆 ${fmt(_clanData.wins)} galibiyet</div>
          </div>
        </div>
...
function renderClanManage(){
  ...
  // Son isim değişikliği zamanı (cooldown için)
  const lastRename = (_clanData && _clanData.lastRenameAt) || 0;
  const cooldownMs = 7 * 24 * 60 * 60 * 1000; // 7 gün
  const elapsed = Date.now() - lastRename;
  const canRename = isAdm || elapsed >= cooldownMs;
  const remainDays = Math.ceil((cooldownMs - elapsed) / (24*60*60*1000));

  box.innerHTML = `
    <div class="clan-card">
      <div class="clan-lbl">✏️ KLAN ADI & ETİKET ${isAdm ? '<span style="color:#FFD740;font-size:9px">👑 ADMİN — SINIRSIZ</span>' : ''}</div>
      <div class="clan-row" style="margin-bottom:6px">
        <input class="clan-in" id="clanRenameTag" maxlength="${isAdm?'12':'5'}" placeholder="Etiket" value="${esc(_clanData.tag||'')}" style="flex:0 0 90px;text-transform:uppercase">
        <input class="clan-in" id="clanRenameName" maxlength="${isAdm?'40':'20'}" placeholder="Klan adı" value="${esc(_clanData.name||'')}">
      </div>
      ${canRename
        ? '<button class="clan-btn p" id="clanRenameBtn" style="width:100%">💾 Adı Güncelle</button>'
        : `<div class="clan-msg" style="color:#ff8fa0;font-size:11px;text-align:center">⏳ ${remainDays} gün sonra tekrar değiştirebilirsin</div>`}
      ${isAdm ? '<div style="font-size:9px;color:#6d7aa8;margin-top:4px;text-align:center">Admin olarak süre kısıtlaması ve uzunluk limitleri seni etkilemez</div>' : ''}
    </div>
    <div class="clan-card">
  ...
  // İsim değiştirme
  const renameBtn = box.querySelector('#clanRenameBtn');
  if(renameBtn) renameBtn.addEventListener('click', async () => {
    const newTag = box.querySelector('#clanRenameTag').value.trim().toUpperCase();
    const newName = box.querySelector('#clanRenameName').value.trim();
    // Admin değilse kurallar geçerli
    if(!isAdm){
      if(newName.length < 3){ alert('Klan adı en az 3 harf olmalı'); return; }
      if(newTag.length < 2 || newTag.length > 5){ alert('Etiket 2–5 harf olmalı'); return; }
    } else {
      if(!newName){ alert('Klan adı boş olamaz'); return; }
      if(!newTag){ alert('Etiket boş olamaz'); return; }
    }
    try{
      const upd = { name: newName, tag: newTag };
      if(!isAdm) upd.lastRenameAt = Date.now(); // admin cooldown'a takılmaz, zaman da yazmaz
      await fdb.update(fdb.ref(db, 'clans/' + C.myClanId), upd);
      if(_clanData){ _clanData.name = newName; _clanData.tag = newTag; if(!isAdm) _clanData.lastRenameAt = Date.now(); }
      alert('✓ Klan adı güncellendi');
      renderMyClan();
