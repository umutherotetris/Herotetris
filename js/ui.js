...
function render(state){
  ...
  // İsim + seviye + kaju (profile yoksa makul varsayılan)
  const lvl  = profile && profile.level ? profile.level : 1;
  const kaju = profile && profile.kaju != null ? profile.kaju : 0;
  setText('pName', displayName);
  setText('pLevel', 'LV.' + lvl);
  setText('pKaju', fmt(kaju));
  setText('kajuBig', fmt(kaju));
...
export function initUI(){
  bind();
  Auth.subscribe(render);   // TEK abonelik → tüm UI buradan
}
