...
function fitCanvas(){
  ...
  const reservedH = 120; // tg-controls + tg-actions + header için
  const availH = Math.min(wrap.clientHeight, window.innerHeight - reservedH) || (window.innerHeight - reservedH);
...
.tv-game{ flex:1; display:flex; flex-direction:column; padding:8px; max-width:640px; margin:0 auto; width:100%; box-sizing:border-box; max-height:100vh; overflow:hidden; }
...
.tg-board-wrap{ flex:1; display:flex; align-items:center; justify-content:center; min-height:0; max-height:calc(100dvh - 180px); overflow:hidden; }
