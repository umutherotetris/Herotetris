...
function clearLines(){
  ...
  if(cleared > 0){
    ...
    G.dropInterval = Math.max(90, 800 - (G.level-1)*70);
    updateHUD();
  }
}
...
function softDrop(){ if(!G.cur || G.over || G.paused) return; if(!collides(G.cur, 0, 1)){ G.cur.y++; G.score += 1; updateHUD(); } else lock(); G.dropAcc = 0; }
function hardDrop(){
  if(!G.cur || G.over || G.paused) return;
  let d = 0; while(!collides(G.cur, 0, 1)){ G.cur.y++; d++; }
  G.score += d * 2; updateHUD(); lock(); G.dropAcc = 0;
}
function rotate(){
  if(!G.cur || G.over || G.paused) return;
  const r = rotateCW(G.cur.matrix);
  for(const k of [0, -1, 1, -2, 2]){   // basit duvar tekmesi
    if(!collides(G.cur, k, 0, r)){ G.cur.matrix = r; G.cur.x += k; return; }
  }
}
function hold(){
...
function fitCanvas(){
  ...
  const cs = Math.floor(avail / COLS);
  G.cellSize = cs;
  ...
  G.cellSize = cs;            // ctx ölçeklendi, hücre CSS px
}
...
function startGame(){
  ...
  G.dropInterval = 800; G.dropAcc = 0; G.last = 0;
  G.over = false; G.paused = false;
  fitCanvas(); updateHUD(); drawSide();
